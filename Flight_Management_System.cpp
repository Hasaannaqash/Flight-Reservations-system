...
#include <sstream> // for the string stream (used to convert into to string)
#include <iomanip> // for organizing output into columns -> setw();
#include <vector>
#include <fstream>
#include <string.h>
#include <stdlib.h>
...
public://public members functions
    void pay_detail()//function declaration and definition for payment method
    {
        ...
        switch (choice1)//switch case
        {
            ...
            case 3://condition
                cout
                        << "Banks Available: \1.West Pac Bank(1) \2.Nabil Bank(2) \3.Standard Chartered Bank(3) \4.AMP Bank(4) \5.Others(5)";
                cout << "\nSelect your bank:";
                cin >> bank;
...
class Person : public Payment //class for managing the functions of a passenger
{
private:
...
public:
    ...
    // (call ONLY when a booking is cancelled)
    void cancel(); //cancel a booking (Menu Option Νο 7)
...
class Booking // class that stores passengers on each flight's queue
{
...
public:
    /* --CREATE NEW QUEUE-- */
    Booking(int queueId); // queueID is the same as the flight number for each flight
...
list<Flight> flist; // Flight storage
list<Person> plist; // Passenger storage
list<Booking> qlist; // books the flight for customer

/* ----FUNCTION OF CLASS Queue---- */

Booking::Booking(int queueId) {
...
void Booking::display(int num) {


    for (std::list<Booking>::iterator i = qlist.begin(); i != qlist.end(); ++i) {
        if (num == i->queueNo) {
            Node *p = new Node;
            p = i->front;
            ...
            if (i->front == NULL) {
                ...
                cout << left << setw(12) << "Passport" << left << setw(12) << "Name" << left << setw(12) << "Surame"
                     << endl;
                while (p != NULL) {
                    cout << left << setw(12) << p->person.getPassport() << left << setw(12) << p->person.getName()
                         << left << setw(12) << p->person.getSurname() << endl;
                    p = p->next;
                }
            }
        }
...
void Flight::addFlight() {
    ...
    do {
        ...
        if (!checkNumber(temp)) {
            ...
            flag = true;
            this->flightNo = atoi(temp.c_str()); //temporary string
        }
    } while (!flag);
    ...
    do {
        getline(cin, temp);
        if ((temp.length() <= 12) && (checkString(temp))) {
            this->from = temp;
            flag = true;
        } else {
    ...
    do {
        getline(cin, temp);
        if ((temp.length() <= 12) && (checkString(temp)) && (temp.compare(this->from))) {
            this->to = temp;
            flag = true;
        } else {
    ...
    Booking q(this->flightNo); // create new queue for the newly added flight
...
void Flight::displayFlightInfo(int num) {
    ...
    for (std::list<Flight>::iterator i = flist.begin(); i != flist.end(); ++i) {
        if (num == i->flightNo) {
            ...
            cout << left << setw(12) << "FLIGHT" << left << setw(12) << "FROM" << left << setw(12) << "TO" << left
                 << setw(12) << "LEAVE" << left << setw(12) << "ARRIVE" << left << setw(12) << "COST" << left
                 << setw(12) << "TYPE" << left << setw(12) << "SEATS" << left << setw(12) << "BOOKED" << endl;
            cout << left << setw(12) << i->flightNo << left << setw(12) << i->from << left << setw(12) << i->to << left
                 << setw(12) << l_time << left << setw(12) << a_time << left << setw(12) << i->cost << left << setw(12)
                 << i->plane_type << left << setw(12) << i->seats << left << setw(12) << i->booked_seats << endl;
...
void Flight::displaySchedule() {
    ...
    cout << left << setw(12) << "FLIGHT" << left << setw(12) << "FROM" << left << setw(12) << "TO" << left << setw(12)
         << "LEAVE" << left << setw(12) << "ARRIVE" << left << setw(12) << "COST" << left << setw(12) << "TYPE" << left
         << setw(12) << "SEATS" << left << setw(12) << "BOOKED" << endl;
...
void Person::book() {
    ...
    int counter = 1; // booking
    string choice; // stores user's choice for adding or not more flights to their flight
    Time tArriving;
    Time tLeaving;
    string Departure;
    string Destination;
        ...
        if (choice == "y" || choice == "Y") {
            ...
            do {
                ...
                if (!checkNumber(temp)) {
                    ...
                    cout << "Duplicate passport number" << endl;
                    flag = false;
                    goto LOOP10;
                } else {
                    flag = true;
        ...
        */
        ...
        if (Flight::checkForSeats(num)) { // check for vacancy
            ...
            if (flag) {
                ...
                for (std::list<int>::iterator i = nums.begin(); i != nums.end(); ++i) {
                    for (std::list<Booking>::iterator i2 = qlist.begin(); i2 != qlist.end(); ++i2) {
                        if (*i == i2->getNo()) {
                            i2->enqueue(*this);
                        }
                    }
...
void Person::bookFromQueue(int num) {  //is called only from cancel()
    ...
    for (std::list<Booking>::iterator it = qlist.begin(); it != qlist.end(); ++it) {
        if (num == it->getNo()) {
            if (!(it->isEmpty())) {
...
void Person::cancel() {
    ...
    bool flightFound = false; //For customer duplicacy
    ...
    if (!plist.empty()) {
        ...
        cout << "\nYour booking was successfully removed! \n" << endl;
    } else {
        cout << "There are no registered clients at the moment!" << endl;
    }
}
...
myFile.open("test.txt");
    ...
    for (std::list<Person>::iterator i = plist.begin(); i != plist.end(); ++i) {
        ...
        if (passport == i->passportNo) {
            ...
            cout << left << setw(12) << "NAME" << left << setw(12) << "SURNAME" << left << setw(12) << "PASSPORT_No"
                 << left << setw(12) << "NATIONALITY" << left << setw(12) << "ADDRESS" << left << setw(12)
                 << "TELEPHONE" << "Payment" << endl;

            myFile >> i->name;


            cout << left << setw(12) << i->name << left << setw(12) << i->surname << left << setw(12) << i->passportNo
                 << left << setw(12) << i->nationality << left << setw(12) << i->address << left << setw(12) << i->tel << i->payment
                 << endl << endl;
            cout << "Your flight(s): ";
